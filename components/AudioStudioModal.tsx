...
// MOVED OUTSIDE: Component for individual speaker input
const SpeakerInput = ({ 
    index, 
    config, 
    setConfig, 
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4 animate-fade-in-down" onClick={onClose}>
            <div className="bg-slate-800 border border-slate-700 w-full max-w-2xl rounded-2xl shadow-2xl p-6 flex flex-col max-h-[90vh]" onClick={e => e.stopPropagation()}>
                ...
                <div className="overflow-y-auto pr-2 space-y-6">
                    ...
                    {/* THIS SECTION IS ALWAYS VISIBLE NOW */}
                    <div className="space-y-4 p-4 rounded-lg bg-slate-900/50 relative">
                         {/* Optional overlay ONLY if gemini isn't active, to grey it out but not hide it */}
                         {voiceMode === 'system' && (
                             <div className="absolute inset-0 bg-slate-900/50 rounded-lg z-10 pointer-events-none"></div>
                         )}
                        ...
                        {/* QUAD SPEAKER GRID - ALWAYS RENDERED */}
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                             <SpeakerInput index={1} config={speakerA} setConfig={setSpeakerA} uiLanguage={uiLanguage} onUpgrade={onUpgrade} />
                             <SpeakerInput index={2} config={speakerB} setConfig={setSpeakerB} uiLanguage={uiLanguage} onUpgrade={onUpgrade} />
                             <SpeakerInput index={3} config={speakerC} setConfig={setSpeakerC} uiLanguage={uiLanguage} onUpgrade={onUpgrade} locked={false} />
                             <SpeakerInput index={4} config={speakerD} setConfig={setSpeakerD} uiLanguage={uiLanguage} onUpgrade={onUpgrade} locked={false} />
                        </div>
                    </div>
                </div>
            </div>
