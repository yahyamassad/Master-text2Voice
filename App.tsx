...
const App: React.FC = () => {
  ...
  // [Sawtli Bug Fix]: This useEffect was causing system voices to stop immediately after starting.
  // The logic seems to trigger a 'stopAll' call during the asynchronous startup of the SpeechSynthesis API.
  // Disabling this feature ensures the core playback functionality works. Users can still manually
  // stop playback by clicking the speaker button again. A more robust implementation of this
  // auto-stop feature may be considered in the future.
  /*
  useEffect(() => {
    if (activePlayer && (audioSourceRef.current || window.speechSynthesis.speaking)) {
      stopAll();
    }
  }, [sourceText, translatedText, voice, pauseDuration, multiSpeaker, speakerA, speakerB, emotion, stopAll]);
  */
  ...
  useEffect(() => {
    try {
      ...
      if (savedSettingsRaw) {
        ...
        // uiLanguage is set on initialization and saved, but not loaded here to prevent loops
      }
      ...
      if (isGeminiVoice) {
        ...
        try {
            if (window.speechSynthesis.speaking) {
                window.speechSynthesis.cancel(); // Defensive cancel
            }
